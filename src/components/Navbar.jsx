...
export default function Navbar() {
  ...
  const navLinkClass = (path) =>
    `px-3.5 py-1.5 rounded-xl text-sm font-semibold transition-all duration-200 flex items-center gap-1.5 ${
      isActive(path)
        ? 'bg-[#7CA9FF] text-slate-950 shadow-md shadow-[#7CA9FF]/20 font-bold'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <Link to="/dashboard" className="flex items-center gap-2.5 group">
          ...
          <div className="flex flex-col">
            <span className="font-heading font-bold text-xl text-white tracking-tight leading-tight flex items-center gap-1">
              Commute<span className="text-[#7CA9FF]">Share</span>
            </span>
            <span className="text-[10px] text-slate-400 font-mono uppercase tracking-widest -mt-0.5">Rapido Mobility</span>
          </div>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            <>
              ...
              <Link
                to="/profile"
                className="flex items-center gap-2.5 pl-2 pr-3.5 py-1.5 rounded-xl bg-slate-900/80 hover:bg-slate-800 transition-all border border-slate-800"
              >
            ...
            <div className="flex items-center gap-2.5">
              ...
              <Link
                to="/signup"
                className="px-5 py-2 text-sm font-bold text-slate-950 bg-[#7CA9FF] hover:bg-[#6697FF] rounded-xl shadow-md shadow-[#7CA9FF]/20 transition-all duration-200 hover:scale-[1.02]"
              >
                Get Started
              </Link>
            </div>
        ...
        <div className="flex md:hidden items-center gap-2">
          ...
          <button
            ...
            className="p-2 rounded-xl text-slate-300 bg-slate-900 border border-slate-800"
          ...
          {user ? (
            <>
              <div className="flex items-center gap-3 p-3 bg-slate-900 rounded-xl border border-slate-800 mb-3">
                <div className="w-9 h-9 rounded-full bg-[#7CA9FF] text-slate-950 font-bold text-sm flex items-center justify-center">
                  {user.name?.[0]?.toUpperCase() || 'U'}
                </div>
                <div>
              ...
              <Link
                ...
                className="block px-3.5 py-2.5 rounded-xl text-slate-200 hover:bg-slate-900 font-medium"
              >
                🔍 Find Ride
              </Link>
              {(user.role === 'driver' || user.role === 'admin') && (
              ...
              <button
                ...
                className="w-full text-left px-3.5 py-2.5 rounded-xl text-rose-400 hover:bg-rose-950/30 font-medium"
              >
                🚪 Logout
              </button>
            </>
            ...
            <div className="space-y-2 pt-2">
              <Link
                to="/login"
                onClick={() => setMobileMenuOpen(false)}
                className="block w-full text-center px-4 py-2.5 text-slate-200 bg-slate-900 rounded-xl font-semibold"
              >
                Log in
              </Link>
              <Link
                to="/signup"
                onClick={() => setMobileMenuOpen(false)}
                className="block w-full text-center px-4 py-2.5 text-slate-950 bg-[#7CA9FF] rounded-xl font-bold"
              >
                Sign up
              </Link>
            </div>
